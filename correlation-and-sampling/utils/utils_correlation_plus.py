...
def generate_short_lists(original_list, max_cfd_length, cfd_size_limit):
    final_result = []
    for lst in original_list:
        if len(lst) <= max_cfd_length-2:
            final_result.append(lst)
        else:
            sublist = combinations(lst, max_cfd_length-2)
