...
def table_column_embedding_split(df, embedder, sentences):
    embeddings_list = []
    for sentence in sentences:
        ...
        for chunk in chunks:
            sentence_embeddings = embedder.encode(chunk, convert_to_tensor=True).cpu()
            all_sentence_embeddings.append(sentence_embeddings)
        all_sentence_embeddings = torch.stack(all_sentence_embeddings)
        average_embedding = torch.mean(all_sentence_embeddings, dim=0)
