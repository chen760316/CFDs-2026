...
def table_column_information(file_path, len_limit):
    ...
    if column_count < 15:
        ...
        # max_cfd_length = math.floor(column_count * len_limit) + 3
    else:
        max_cfd_length = column_count // 2
        # max_cfd_length = column_count // 2 + 2
    print("*" * 185)
    return df, max_cfd_length
...
def calculate_columns_from_PCA(projection, matrix_column_limit, df, max_cfd_length):
    count = 0
    clusters = []
    columns = []
    while count < matrix_column_limit:
        positive_rows = np.where(projection[:, count] > 0.15)[0]
        negative_rows = np.where(projection[:, count] < -0.15)[0]
        zero_rows = np.where(abs(projection[:, count]) < 0.15)[0]
        positive_values = projection[positive_rows, count]
        sorted_indices = np.argsort(positive_values)
        negative_values = projection[negative_rows, count]
        sorted_indices_negative = np.argsort(negative_values)[::-1]
        sorted_positive_rows = positive_rows[sorted_indices]
        sorted_negative_rows = negative_rows[sorted_indices_negative]
        positive = np.append(sorted_positive_rows, zero_rows)
        negative = np.append(sorted_negative_rows, zero_rows)
        clusters.append(positive)
        clusters.append(negative)
        for lst in clusters:
            column_lst = df.columns[lst].tolist()
            if len(column_lst) <= max_cfd_length:
                columns.append(column_lst)
            else:
                columns.append(column_lst[-max_cfd_length:])
...
def print_initial_instance(datasets_path, result):
...
def generate_related_columns_from_lasso(embedding, alpha):
    ...
    for column_name in embedding.columns:
        ...
        print(target_col + "和剩余属性的复相关系数：", corr)
...
def nonlinear_correlated_columns_from_from_KernelPCA(embedding_matrix, df, max_cfd_length, matrix_column_limit):
    pca_model = KernelPCA(kernel='rbf', gamma=0.5, n_components=matrix_column_limit)
    ...
    while count < reduction.shape[1]:
        ...
        positive_values = reduction[positive_rows, count]
        sorted_indices = np.argsort(positive_values)
        negative_values = reduction[negative_rows, count]
        sorted_indices_negative = np.argsort(negative_values)[::-1]
        sorted_positive_rows = positive_rows[sorted_indices]
        sorted_negative_rows = negative_rows[sorted_indices_negative]
        clusters.append(sorted_positive_rows)
        clusters.append(sorted_negative_rows)
        for lst in clusters:
            column_lst = df.columns[lst].tolist()
            if len(column_lst) <= max_cfd_length:
                columns.append(column_lst)
...
def gen_nonlinear_rel_from_forest(matrix, max_cfd_length):
    col_length = matrix.shape[1]
    nonlinear_columns = []
    for target_col in range(col_length):
        y = matrix.iloc[:, target_col]
        other_cols = [i for i in range(col_length) if i != target_col]
        rf = RandomForestRegressor(n_estimators=100, random_state=42)
        rf.fit(matrix.iloc[:, other_cols], y)
        feature_importances = rf.feature_importances_
        sorted_indices = np.argsort(feature_importances)[::-1]
        selected_features = [other_cols[idx] for idx in sorted_indices[:max_cfd_length-1]]
        selected_features.append(target_col)
        nonlinear_columns.append(selected_features)
    return nonlinear_columns
